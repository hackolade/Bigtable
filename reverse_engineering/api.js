...
module.exports = {
  ...
  getDbCollectionsData: function (data, logger, cb) {
    let { recordSamplingSettings, fieldInference, includeEmptyCollection } =
      data;
    ...
    async.map(
      ...
      (err, res) => {
        if (err) {
          return logError(logger, cb)(err, "Get data error");
        } else {
          return cb(err, res);
        }
      },
...
const splitTableName = (name) =>
  name.indexOf(":") !== -1 ? name.split(":") : ["", name];

const getNamespacesFromTables = (tables) => {
  return Promise.resolve(
    _.uniq(tables.map((tableName) => splitTableName(tableName || "").shift())),
  );
};
    ...
    .then(
      ...
      (err) => {
        ...
        if (areNamespacesNotAllowed) {
          return getTableNames(connectionInfo, logger).then(
            getNamespacesFromTables,
          );
        } else {
...
function handleRows(rows) {
  ...
  rows.forEach((item) => {
    ...
    let handledColumn = handleColumn(
      item,
    ...
    if (colFamConfig && colFamConfig.length) {
      colFamConfig.forEach((prop) => {
        switch (prop.propertyType) {
          case "number":
            customSchema.properties[item.name][prop.propertyKeyword] = Number(
              item[prop.schemaKeyword],
            );
            break;
          case "boolean":
            customSchema.properties[item.name][prop.propertyKeyword] =
              getBoolean(item[prop.schemaKeyword]);
            break;
          default:
            customSchema.properties[item.name][prop.propertyKeyword] =
              item[prop.schemaKeyword];
